...
SELECT name, region FROM country WHERE continent = 'North America' OR region = 'South America';
...
SELECT gnp, (gnp / population) AS gnppercapita FROM country WHERE continent = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT indepyear, (2021 - indepyear) AS yearssinceindependence FROM country WHERE indepyear IS NOT NULL;
