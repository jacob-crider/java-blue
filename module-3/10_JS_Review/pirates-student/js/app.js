...
function loss(message) {
    ...
    // setTimeout (methodToCallWhenTimerExpires, millisecondsToWait)
    setTimeout( () => {
...
function resetGame() {
    ...
    createObstacles();
    resetBoat();
    //sets the starting location of boat and treasure
...
function createObstacles() {
    // Get a reference to all squares
    const rows = document.querySelectorAll('#frame .row');

    Array.from(rows).forEach((row, rowIndex) => { // makes rows come out as a nodeList
    ...
    // For each square call add obstacle
}
...
function handleTick() {
    ...
    pirates.forEach(pirate => {
        ...
        if (isPirateWin(newLocation)) {
            loss("You were sunk by a pirate!");
